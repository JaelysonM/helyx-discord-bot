...
module.exports.run = async (client, message, args, command) => {
    ...
    let titulo = new MessageEmbed()
        .setAuthor(`Spoiler`, client.user.avatarURL)
        .setDescription(`\n\u200bQual título você deseja para enviar o spoiler?`)
    ...
    message.author.send(titulo).then(async msg => {
        ...
        collector.on('collect', a => {
            ...
            let desc = new MessageEmbed()
                .setAuthor(`Spoiler`, client.user.avatarURL)
                .setDescription(`\n\u200bQual descrição você deseja para enviar o spoiler?`)
                .setFooter(`© RedeShelds - Todos os direitos reservados!`)
                .setColor(`#00FFFF`)
            ...
            collector.on('collect', async a => {
                ...
                collector.on('collect', async a => {
                    ...
                    if (imagem == `nao`) {
                        ...
                        await message.author.send(`Preview do Spoiler`, embed)

                        message.author.send(`🚫 Para enviar este spoiler, digite \`confirmar\`, para cancelar digite \`cancelar\`.`)
                        var collector = message.author.dmChannel.createMessageCollector(a => a.author.id == message.author.id, { time: 120000, max: 1 })
                        collector.on('collect', async a => {
                        ...
                        collector.on('collect', async a => {
                            ...
                            if (cancelamento == `confirmar`) {
                                ...
                                message.author.send(`Spoiler enviado com sucesso!`)
...
exports.help = {
    name: 'spoiler',
    description: 'Cria uma mensagem como spoiler no servidor;'
}